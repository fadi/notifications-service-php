...
// Load .env variables (required for shared hosting where getenv() doesn't auto-load .env)
$envPath = __DIR__ . '/.env';
...
/**
 * Notification Service - Single File Version with Mailgun Integration
 *
 * Setup:
 * 1. Copy .env.example to .env and fill in your values
 * 2. Run the database migration: schema.sql
 * 3. Deploy index.php to your web root
 *
 * Usage:
 * curl -X POST http://localhost:8000/send \
 *   -H "Content-Type: application/json" \
 *   -H "X-API-Key: your-secret-key" \
 *   -d '{"recipient_id":"user123","template_name":"welcome","variable_data":{"name":"John","product":"App"}}'
 */
...
$config = [
    ...
    'debug'           => filter_var(optionalEnv('APP_DEBUG', 'false'), FILTER_VALIDATE_BOOLEAN),
    ...
    'from_email'      => optionalEnv('FROM_EMAIL', 'noreply@yourdomain.com'),
...
if (defined('TESTING')) { return; }

$logger      = new Logger($config['log_file'], $config['debug']);
$rateLimiter = new RateLimiter($config['rate_limit'], $logger);
$db          = new Database($config, $logger);
$templates   = new TemplateEngine($config['templates'], $config['email_subjects']);
...
$basePrefix = optionalEnv('BASE_PREFIX', '');
...
try {
    if ($method === 'GET' && $path === '/health') {
        respondJson(200, [
            ...
            'mailgun_configured' => $emailSender->isConfigured(),
        ]);
    }

    if ($method === 'POST' && $path === '/send') {
        authenticate($config['api_key'], $logger);
        $clientId = getClientId();
        if (!$rateLimiter->checkLimit($clientId)) {
            header('X-RateLimit-Limit: ' . $config['rate_limit']);
            header('X-RateLimit-Remaining: 0');
            respondJson(429, ['error' => 'Rate limit exceeded. Maximum ' . $config['rate_limit'] . ' requests per minute.']);
        }
        $body = file_get_contents('php://input');
        $data = json_decode($body, true);
        if (!is_array($data)) { respondJson(400, ['error' => 'Invalid JSON']); }
        $validated = $validator->validateSendRequest($data);
        $result = $service->send($validated['recipient_id'], $validated['template_name'], $validated['variable_data']);
        header('X-RateLimit-Limit: ' . $config['rate_limit']);
        header('X-RateLimit-Remaining: ' . $rateLimiter->getRemaining($clientId));
        respondJson(200, ['status' => 'success', 'data' => $result]);
    }

    respondJson(404, ['error' => 'Not found']);

} catch (Exception $e) {
    $logger->error('Request failed', ['error' => $e->getMessage(), 'file' => $e->getFile(), 'line' => $e->getLine()]);
    if (strpos($e->getMessage(), 'Validation failed') === 0) {
        $errorData = json_decode(substr($e->getMessage(), 19), true);
        respondJson(400, ['error' => 'Validation failed', 'details' => $errorData]);
    }
    if (strpos($e->getMessage(), 'Template') !== false && strpos($e->getMessage(), 'not found') !== false) {
